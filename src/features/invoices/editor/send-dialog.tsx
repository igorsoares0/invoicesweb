    ...
    <Dialog open={open} onOpenChange={(next) => !pending && onOpenChange(next)}>
      <DialogContent className="gap-0 p-0 sm:max-w-[560px]">
        <DialogHeader className="px-6 pt-6 pb-4">
          <DialogTitle className="text-[17px] font-semibold">Send invoice</DialogTitle>
          <DialogDescription>
            {number} · {formatMoney(total, currency)} · due {formatShortDate(dueDate)}
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-3 px-6 pb-5 text-[14px]">
          {error ? <ErrorBanner>{error}</ErrorBanner> : null}
          <p>
            Marking the invoice as sent publishes a private link you can share with{" "}
            <strong className="font-semibold">{clientEmail ?? "your client"}</strong>. They can view it and download the PDF
            without an account.
          </p>
