...
import type { ClientDto, InvoiceDto, ProductDto } from "@/lib/api-types";
import { COMMON_CURRENCIES, currencyOptions, currencySymbol } from "@/lib/currencies";
import { buildInvoiceView, type PartyInput } from "@/lib/documents/view";
...
export function InvoiceEditor({
  ...
  const { draft, update, invoice, status, savedAt, errors, issues, lines, totals, flush } = useInvoiceDraft(initial);
  ...
  const view = useMemo(
    () =>
      buildInvoiceView({
        ...
        dueDate: draft.dueDate >= draft.issueDate ? draft.dueDate : draft.issueDate,
        issuer,
        billTo: client,
    ...
    [client, draft, invoice.number, issuer, lines, totals],
  );
  ...
  async function downloadPdf() {
    ...
    link.href = `/api/v1/invoices/${invoice.id}/pdf?download=1`;
    link.click();
  }
  ...
  async function send(): Promise<string | null> {
    ...
    try {
      await api.post<InvoiceDto>(`/invoices/${invoice.id}/send`, {});
      toast.success(`${invoice.number} marked as sent`);
      setSendOpen(false);
      router.refresh();
      return null;
    } catch (error) {
      return error instanceof ApiClientError ? error.message : "Couldn't send the invoice";
    }
  }
  ...
  const preview = (
    <div className="overflow-hidden rounded-md border bg-white shadow-[0_2px_8px_rgba(0,0,0,0.06)]">
      <FittedDocument>
        <InvoiceDocument view={view} template={draft.template} />
      </FittedDocument>
    </div>
  );
  ...
  const headerFields = (
    <section className="grid gap-4 rounded-lg border bg-card px-5 py-4 md:grid-cols-[minmax(0,1fr)_148px_148px]">
      ...
      <div>
        <Label htmlFor="due-date" className={cn("mb-1.5", fieldErrors.dueDate && "text-destructive")}>
          Due date
        </Label>
        <Input
          id="due-date"
          ...
          onChange={(event) => update((current) => ({ ...current, dueDate: event.target.value }))}
        />
    ...
    <div className="flex flex-col gap-3">
      <div>
        ...
        <Textarea
          id="notes"
          rows={3}
          placeholder="Thanks for your business. Bank details are below."
          value={draft.notes}
      ...
      <div>
        ...
        <Textarea
          id="terms"
          rows={2}
          placeholder="Late payments accrue 1% per month."
          value={draft.terms}
    ...
    <>
      ...
      <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center justify-between gap-3 border-b bg-card px-4 sm:px-6">
        <div className="flex min-w-0 items-center gap-2.5 text-[13px]">
          ...
          <InvoiceStatusBadge status="DRAFT" className="hidden sm:inline-flex" />
      ...
      {phone ? (
        <div className="flex border-b bg-card px-4" role="tablist" aria-label="Editor view">
          ...
          <InvoiceStatusBadge status="DRAFT" className="my-auto ml-auto" />
        </div>
      ) : null}
            ...
            <>
              ...
              {phone ? (
                <section aria-label="Items" className="flex flex-col gap-2.5">
                  ...
                  <ol aria-label="Invoice lines" className="flex flex-col gap-2.5">
      ...
      {!wide && !phone ? (
        <Sheet open={previewOpen} onOpenChange={setPreviewOpen}>
          <SheetContent side="right" className="w-[460px] overflow-y-auto bg-canvas-2 p-5 sm:max-w-[460px]">
            ...
            <SheetDescription className="sr-only">How the invoice will look</SheetDescription>
            <TemplatePicker
              template={draft.template}
              color={draft.color}
              onChange={(patch) => update((current) => ({ ...current, ...patch }))}
