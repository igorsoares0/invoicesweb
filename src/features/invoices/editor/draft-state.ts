import type { DocumentTemplate, InvoiceDto } from "@/lib/api-types";
...
export function toDraft(invoice: InvoiceDto): DraftState {
  return {
 ...
 */
export function toPayload(draft: DraftState): { payload: Record<string, unknown>; errors: FieldErrors } {
  ...
  if (!isValidIsoDate(draft.dueDate)) fail("dueDate", "Enter a valid date");
