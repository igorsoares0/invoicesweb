...
export function ConvertDialog({
  ...
  async function convert() {
    const result = await submit(() =>
      api.post<ConvertEstimateResultDto>(`/estimates/${estimate.id}/convert`, { issueDate, dueDate, send: sendNow }),
    );
    if (!result) return;
    ...
    <Dialog open={open} onOpenChange={(next) => !pending && onOpenChange(next)}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {estimate.number} stays untouched and is marked <strong className="font-semibold text-[#6d28d9]">Converted</strong>.
          </DialogDescription>
        </DialogHeader>
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <Input id="convert-issue" type="date" value={issueDate} onChange={(event) => setIssueDate(event.target.value)} />
              {error("issueDate") ? <p className="mt-1 text-[12.5px] text-destructive">{error("issueDate")}</p> : null}
            </div>
            <div>
              <Label htmlFor="convert-due" className="mb-1.5">
                Due date {terms !== null && terms >= 0 ? <span className="font-normal text-muted-2">· Net {terms}</span> : null}
              </Label>
              <Input
                id="convert-due"
                type="date"
