...
import { toEditable, type EditableDocument } from "./kinds";
import { AUTOSAVE_DELAY, useDocumentDraft } from "./use-document-draft";
...
describe("useInvoiceDraft", () => {
  it("batches quick edits into one save and computes totals locally meanwhile", async () => {
    ...
    const { result } = renderHook(() => useDocumentDraft(toEditable(invoice)));
  ...
  it("never sends a save while the previous one is still on its way", async () => {
    ...
    expect(result.current.document.notes).toBe("second");
    expect(result.current.status).toBe("saved");
  });
  ...
  it("flush saves immediately and returns the server copy", async () => {
    ...
    let flushed: EditableDocument | null = null;
    await act(async () => {
